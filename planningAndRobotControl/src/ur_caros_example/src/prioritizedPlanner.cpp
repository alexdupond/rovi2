	...
	for(size_t i = 0; i < qVec.size()-1; i++)
    {
        ...
        if (t.getTime() >= MAXTIME) {
           // cout << "Notice: max time of " << MAXTIME << " seconds reached." << endl;
            ...
            t.pause(); 
            cout << "Optimized path of length " << path.size() << " found in " << t.getTime() << " seconds." << endl;
		    // csvfile.open(file_name, ios::out | ios::app);
		    // csvfile << path.size() << ", " << t.getTime() << ", ";
		    // csvfile.close();
        }
...
bool PrioritizedPlanner::calculateDynamicRRTPath(const vector<rw::math::Q> &qVec, rw::trajectory::QPath& qPathRob1, rw::trajectory::QPath& result, caros::SerialDeviceSIProxy* rob1, caros::SerialDeviceSIProxy* rob2){
    _path_1 = qPathRob1; 
   ...
   // cout << "Total path of path 1 is = " << qPathRob1.size() << " and number of timesteps = " << _timesteps.size() << endl; 
    ...
    //    cout << "Depth initialized to = " << depth << endl;
        ...
        while (true) {
            ...
            {
                ros::spinOnce();
                rob1->movePtp(rob1->getQ());
                rob2->movePtp(rob2->getQ());
                hej = 0;
            }
            ...
            if(t.getTime() > 15){
                ...
                // csvfile << 0 << ", " << 0  << endl; 
                // csvfile.close();
                cout << "TIMEOUT WHEN PLANNING!" << endl; 
                i--;
...
{   
    ...
    cout << "Distance to goal configuration = " << distGoal << endl; 
    ...
    if (distGoal <= _extend) {
        ...
        if (!inCollision(detector, qNew, qRobNext) && !inCollision(detector, qRobPrev, qRobNext, qNear, qNew, 0.005))
        {
            cout << "Adding new node to tree - Size = " << tree.size() << endl; 
            tree.add(qNew, qNearNode, 1);
            return Advanced;
        } else {
            return Trapped;
