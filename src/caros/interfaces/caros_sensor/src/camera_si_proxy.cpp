...
void CameraSIProxy::shootMultiple()
{
  std::unique_lock<std::mutex> lk(mutex_);
  image_transport::ImageTransport it(nodehandle_);
  img_sub_ = it.subscribeCamera(ros_namespace_ + "/image_rect_color", 1, &CameraSIProxy::handleImage, this);
...
cv::Matx33d CameraSIProxy::getIntrinsicMatrix()
{
  std::lock_guard<std::mutex> lock(mutex_);
  cv::Matx33d intrinsic_mat = cam_model_.fullIntrinsicMatrix();
  return intrinsic_mat;
}

cv::Mat_<double> CameraSIProxy::getDistortionCoefficients()
{
  std::lock_guard<std::mutex> lock(mutex_);
  cv::Mat_<double> dist_coeffs = cam_model_.distortionCoeffs();
  return dist_coeffs;
}
