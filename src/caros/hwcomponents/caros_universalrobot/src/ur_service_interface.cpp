...
{
  if (srv_ur_servo_stop_ || srv_ur_servo_update_ || srv_ur_servo_q_ || srv_ur_force_mode_start_ ||
      srv_ur_force_mode_update_ || srv_ur_force_mode_stop_ || srv_ur_set_payload_ || srv_ur_set_io_)
  {
    ROS_WARN_STREAM(
  ...
  srv_ur_servo_stop_ = nodehandle_.advertiseService("servo_stop", &URServiceInterface::urServoStopHandle, this);
  ROS_ERROR_STREAM_COND(!srv_ur_servo_stop_, "The servo_stop service is empty!");

  srv_ur_servo_update_ = nodehandle_.advertiseService("servo_update", &URServiceInterface::urServoUpdateHandle, this);
  ROS_ERROR_STREAM_COND(!srv_ur_servo_update_, "The servo_update service is empty!");
  ...
  if (srv_ur_servo_stop_ && srv_ur_servo_update_ && srv_ur_servo_q_ && srv_ur_force_mode_start_ &&
      srv_ur_force_mode_update_ && srv_ur_force_mode_stop_ && srv_ur_set_payload_)
  {
...
bool URServiceInterface::urServoUpdateHandle(caros_universalrobot::UrServiceServoUpdate::Request& request,
                                             caros_universalrobot::UrServiceServoUpdate::Response& response)
{
  rw::math::Q target = caros::toRw(request.target);
  response.success = urServoUpdate(target);
  return true;
}
