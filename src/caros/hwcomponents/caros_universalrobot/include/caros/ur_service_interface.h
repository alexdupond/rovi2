...
class URServiceInterface
{
 public:
  ...
  /**
   * @brief update servo target joint q
   */
  virtual bool urServoUpdate(const rw::math::Q& target) = 0;

  /**
   * @brief servo to joint q target
   */
  virtual bool urServoQ(const rw::math::Q& target, const float time, const float lookahead_time,
                        const float gain) = 0;
 ...
 private:
  ...
  bool urServoUpdateHandle(caros_universalrobot::UrServiceServoUpdate::Request& request,
 ...
 protected:
  ...
  ros::ServiceServer srv_ur_servo_update_;
